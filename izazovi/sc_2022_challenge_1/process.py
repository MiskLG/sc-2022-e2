...
def count_blood_cells(image_path):
    ...
    for i in range(0, len(contours)):
        ...
        if not (centers[0][0] < 30 or centers[0][0] > 610 or centers[0][1] < 30 or centers[0][1] > 450):
            cv.circle(img_binary, centers[0], 45, (151, 113, 222), cv.FILLED)
    show_image(img_binary)
    ...
    dist_8u = sure_fg.astype('uint8')
    _, contours, _ = cv.findContours(dist_8u, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
    for i in range(len(contours)):
        cv.drawContours(markers, contours, i, (i + 1), -1)

    markers = cv.circle(markers, (15, 15), 5, len(contours) + 1, -1)
    markers = cv.watershed(img_base, markers)
    img_base[markers == -1] = [0, 0, 255]
...
def get_binary_image2(img_base):
    ...
    image_ada = cv.GaussianBlur(image_ada, (11, 11), 2)
    image_ada_bin = cv.adaptiveThreshold(image_ada, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 151, 3)
